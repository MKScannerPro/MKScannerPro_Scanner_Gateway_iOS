...
@implementation MKSPMQTTSSLForDeviceView
...
- (void)sp_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        ...
        if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate sp_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate sp_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate sp_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate sp_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate sp_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKScannerPro", @"MKSPMQTTSSLForAppView", @"sp_switchSelectedIcon.png") : LOADICON(@"MKScannerPro", @"MKSPMQTTSSLForAppView", @"sp_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKScannerPro", @"MKSPMQTTSSLForAppView", @"sp_switchUnselectedIcon.png") forState:UIControlStateNormal];
