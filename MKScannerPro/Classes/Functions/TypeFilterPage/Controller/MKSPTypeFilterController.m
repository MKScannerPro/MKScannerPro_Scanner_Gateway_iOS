...
@interface MKSPTypeFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSPTypeFilterModel *dataModel;

@end

@implementation MKSPTypeFilterController

- (void)dealloc {
    NSLog(@"MKSPTypeFilterController销毁");
}
...
- (void)rightButtonMethod {
    ...
    [MKSPServerInterface sp_configBeaconTypeFilter:self.dataModel
...
- (void)readDataFromServer {
    ...
    [MKSPServerInterface sp_readBeaconTypeFilterDatasWithDeviceID:self.deviceModel.deviceID
                                                       macAddress:self.deviceModel.macAddress
                                                            topic:[self.deviceModel currentSubscribedTopic]
                                                         sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPTypeFilterController", @"sp_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSPTypeFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPTypeFilterModel alloc] init];
    }
    return _dataModel;
}
