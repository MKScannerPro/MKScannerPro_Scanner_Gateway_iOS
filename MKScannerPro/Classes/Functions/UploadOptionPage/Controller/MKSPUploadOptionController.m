...
@interface MKSPUploadOptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSPUploadOptionModel *dataModel;

@end

@implementation MKSPUploadOptionController

- (void)dealloc {
    NSLog(@"MKSPUploadOptionController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Beacon type filter
        MKSPTypeFilterController *vc = [[MKSPTypeFilterController alloc] init];
    ...
    if (indexPath.section == 2) {
        ...
        MKSPFilterConditionController *vc = [[MKSPFilterConditionController alloc] init];
        vc.deviceModel = self.deviceModel;
    ...
    if (indexPath.section == 4 && indexPath.row == 0) {
        ...
        MKSPDuplicateDataFilterController *vc = [[MKSPDuplicateDataFilterController alloc] init];
        vc.deviceModel = self.deviceModel;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 5 && indexPath.row == 0) {
        //Upload data option
        MKSPUploadDataOptionController *vc = [[MKSPUploadDataOptionController alloc] init];
        vc.deviceModel = self.deviceModel;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)mk_filterConditionsChanged:(NSInteger)conditionIndex {
    mk_sp_scanFilterConditionShip ship = mk_sp_scanFilterConditionShipOR;
    if (conditionIndex == 0) {
        ship = mk_sp_scanFilterConditionShipAND;
    }
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSPServerInterface sp_configScanFilterConditions:ship deviceID:self.deviceModel.deviceID macAddress:self.deviceModel.macAddress topic:[self.deviceModel currentSubscribedTopic] sucBlock:^(id  _Nonnull returnData) {
...
- (void)readDataFromServer {
    ...
    [MKSPServerInterface sp_readScanFilterConditionsWithDeviceID:self.deviceModel.deviceID
...
- (MKSPUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPUploadOptionModel alloc] init];
    }
    return _dataModel;
}
