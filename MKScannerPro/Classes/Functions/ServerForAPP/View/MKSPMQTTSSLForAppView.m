...
@implementation MKSPMQTTSSLForAppView
...
- (void)sp_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        ...
        if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate sp_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //Client Cert File
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate sp_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sp_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate sp_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPMQTTSSLForAppViewModel.class]) {
        return;
    }
