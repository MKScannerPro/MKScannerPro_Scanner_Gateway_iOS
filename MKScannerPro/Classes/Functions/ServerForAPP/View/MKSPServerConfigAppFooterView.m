//
//  MKSPServerConfigAppFooterView.m
...
@interface MKSPServerConfigAppFooterView ()<MKSPMQTTGeneralParamsViewDelegate,
...
@implementation MKSPServerConfigAppFooterView
...
- (void)sp_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(sp_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate sp_mqtt_serverForApp_certificateChanged:certificate];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKSPServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    ...
    MKSPMQTTSSLForAppViewModel *sslModel = [[MKSPMQTTSSLForAppViewModel alloc] init];
...
- (MKSPMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSPMQTTSSLForAppView alloc] init];
