...
@interface MKSPLedSettingModel : NSObject<sp_indicatorLightStatusProtocol>
...
- (void)mk_textSwitchCellStatusChanged:(BOOL)isOn index:(NSInteger)index {
    ...
    if (index == 2) {
        ...
        self.settingModel.wifi_connecting = isOn;
        return;
    }
    if (index == 3) {
        //WIFI connected indicator
        self.settingModel.wifi_connected = isOn;
        return;
    }
}
...
- (void)confirmButtonPressed {
    ...
    [MKSPServerInterface sp_configIndicatorLightStatus:self.settingModel
...
- (void)readDataFromServer {
    ...
    [MKSPServerInterface sp_readIndicatorLightStatusWithDeviceID:self.deviceModel.deviceID
                                                      macAddress:self.deviceModel.macAddress
                                                           topic:[self.deviceModel currentSubscribedTopic]
                                                        sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)updateCellData:(NSDictionary *)dic {
    ...
    self.settingModel.wifi_connecting = ([dic[@"server_connecting"] integerValue] == 1);
    self.settingModel.wifi_connected = ([dic[@"server_connected"] integerValue] == 1);
...
- (void)loadSectionDatas {
    ...
    cellModel3.index = 2;
    cellModel3.msg = @"WIFI connecting indicator";
    cellModel3.isOn = self.settingModel.wifi_connecting;
    [self.dataList addObject:cellModel3];
    
    MKTextSwitchCellModel *cellModel4 = [[MKTextSwitchCellModel alloc] init];
    cellModel4.index = 3;
    cellModel4.msg = @"WIFI connected indicator";
    cellModel4.isOn = self.settingModel.wifi_connected;
