...
@property (nonatomic, strong)MKSPServerForAppModel *dataModel;

@property (nonatomic, strong)MKSPServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKSPServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKSPServerForAppController

- (void)dealloc {
    NSLog(@"MKSPServerForAppController销毁");
...
- (void)leftButtonMethod {
    ...
    [alertView showAlertWithTitle:@"" message:msg notificationName:@"mk_sp_needDismissAlert"];
}
...
- (void)serverManagerStateChanged {
    if ([MKSPMQTTServerManager shared].state == MKMQTTSessionManagerStateConnecting) {
        [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
        [self startReceiveTimer];
        self.leftButton.enabled = NO;
        return;
    }
    if ([MKSPMQTTServerManager shared].state == MKMQTTSessionManagerStateConnected) {
    ...
    if ([MKSPMQTTServerManager shared].state == MKMQTTSessionManagerStateError) {
        [self connectFailed];
        return;
    }
}
...
- (MKSPServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSPServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSPServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKSPServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSPServerConfigAppFooterViewModel alloc] init];
    }
