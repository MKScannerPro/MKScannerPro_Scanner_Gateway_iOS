...
@interface MKSPASettingController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKSPASettingController

- (void)dealloc {
    NSLog(@"MKSPASettingController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        vc.protocol = [[MKSPASettingsProtocolModel alloc] init];
    ...
    if (indexPath.section == 0 && indexPath.row == 5) {
        //System time
        MKSPASystemTimeController *vc = [[MKSPASystemTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //OTA
        MKSPAOTAController *vc = [[MKSPAOTAController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //Device information
        MKSPADeviceInfoController *vc = [[MKSPADeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)saveDeviceLocalName {
    ...
    [MKSPDeviceDatabaseManager updateLocalName:self.localNameAsciiStr macAddress:[MKSPDeviceModeManager shared].macAddress sucBlock:^{
...
- (void)resetDevice {
    ...
    [MKSPAMQTTInterface spa_configDeviceResetWithDeviceID:[MKSPDeviceModeManager shared].deviceID
                                               macAddress:[MKSPDeviceModeManager shared].macAddress
...
- (void)rebootDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKSPAMQTTInterface spa_rebootDeviceWithDeviceID:[MKSPDeviceModeManager shared].deviceID
                                          macAddress:[MKSPDeviceModeManager shared].macAddress
                                               topic:[MKSPDeviceModeManager shared].subscribedTopic
                                            sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPASettingController", @"sp_editIcon.png")
