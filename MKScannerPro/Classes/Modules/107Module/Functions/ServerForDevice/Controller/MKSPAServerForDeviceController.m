//
//  MKSPAServerForDeviceController.m
...
@interface MKSPAServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKSPAServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKSPAServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKSPAServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKSPAServerConfigDeviceFooterViewModel *sslParamsModel;
...
@implementation MKSPAServerForDeviceController

- (void)dealloc {
    NSLog(@"MKSPAServerForDeviceController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    if (self.connectTimer) {
        dispatch_cancel(self.connectTimer);
    }
    [MKSPAMQTTManager singleDealloc];
}
...
#pragma mark - MKSPAServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)spa_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)spa_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)spa_mqtt_serverForDevice_qosChanged:(NSInteger)qos {
...
- (void)spa_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)spa_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)spa_mqtt_serverForDevice_timeZoneChanged:(NSInteger)timeZone {
...
- (void)receiveDeviceNetState:(NSNotification *)note {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKSPAReceiveDeviceNetStateNotification
                                                  object:nil];
    if (self.connectTimer) {
        dispatch_cancel(self.connectTimer);
    }
...
- (void)configServerParamsComplete {
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                  ...
                                                  object:nil];
    NSString *topic = @"";
    if (ValidStr([MKSPAMQTTManager shared].subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKSPAMQTTManager shared].subscribeTopic;
    }else {
        topic = self.dataModel.publishTopic;
    }
    [self.progressView show];
    [[MKSPAMQTTManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (MKSPAServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSPAServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKSPAServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSPAServerConfigDeviceFooterViewModel alloc] init];
    }
