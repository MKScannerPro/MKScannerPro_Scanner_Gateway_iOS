//
//  MKSPAServerConfigDeviceFooterView.m
...
@implementation MKSPAServerConfigDeviceFooterViewModel
@end
...
@interface MKSPAServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKSPAMQTTSSLForDeviceViewDelegate,
MKSPAServerConfigDeviceSettingViewDelegate>
...
@property (nonatomic, strong)MKSPAMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKSPAServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKSPAServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate spa_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_qosChanged:)]) {
        [self.delegate spa_mqtt_serverForDevice_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spa_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spa_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spa_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKSPAMQTTSSLForDeviceViewDelegate
- (void)spa_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate spa_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)spa_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate spa_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)spa_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spa_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)spa_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spa_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)spa_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spa_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKSPAServerConfigDeviceSettingViewDelegate
- (void)spa_mqtt_deviecSetting_deviceIDChanged:(NSString *)deviceID {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spa_mqtt_serverForDevice_textFieldValueChanged:deviceID textID:3];
    }
}
...
- (void)spa_mqtt_deviecSetting_ntpURLChanged:(NSString *)url {
    if ([self.delegate respondsToSelector:@selector(sp_mqtta_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spa_mqtt_serverForDevice_textFieldValueChanged:url textID:4];
    }
}

- (void)spa_mqtt_deviecSetting_timeZoneChanged:(NSInteger)timeZone {
    if ([self.delegate respondsToSelector:@selector(spa_mqtt_serverForDevice_timeZoneChanged:)]) {
        [self.delegate spa_mqtt_serverForDevice_timeZoneChanged:timeZone];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKSPAServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPAServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKSPAMQTTSSLForDeviceViewModel *sslModel = [[MKSPAMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKSPAServerConfigDeviceSettingViewModel *settingModel = [[MKSPAServerConfigDeviceSettingViewModel alloc] init];
...
- (MKSPAMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSPAMQTTSSLForDeviceView alloc] init];
...
- (MKSPAServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKSPAServerConfigDeviceSettingView alloc] init];
