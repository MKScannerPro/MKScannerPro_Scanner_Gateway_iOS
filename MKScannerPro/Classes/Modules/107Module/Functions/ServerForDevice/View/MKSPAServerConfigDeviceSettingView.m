//
//  MKSPAServerConfigDeviceSettingView.m
...
@implementation MKSPAServerConfigDeviceSettingViewModel
@end

@interface MKSPAServerConfigDeviceSettingView ()
...
@implementation MKSPAServerConfigDeviceSettingView
...
- (void)timeZoneButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(spa_mqtt_deviecSetting_timeZoneChanged:)]) {
            [self.delegate spa_mqtt_deviecSetting_timeZoneChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPAServerConfigDeviceSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPAServerConfigDeviceSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)deviceIDField {
    if (!_deviceIDField) {
        ...
        _deviceIDField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spa_mqtt_deviecSetting_deviceIDChanged:)]) {
                [self.delegate spa_mqtt_deviecSetting_deviceIDChanged:text];
            }
        };
...
- (MKTextField *)ntpUrlField {
    if (!_ntpUrlField) {
        ...
        _ntpUrlField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spa_mqtt_deviecSetting_ntpURLChanged:)]) {
                [self.delegate spa_mqtt_deviecSetting_ntpURLChanged:text];
            }
        };
