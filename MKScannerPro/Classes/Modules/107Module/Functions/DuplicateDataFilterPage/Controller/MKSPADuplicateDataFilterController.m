//
//  MKSPADuplicateDataFilterController.m
...
@interface MKSPADuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSPADuplicateDataFilterModel *dataModel;

@end

@implementation MKSPADuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKSPADuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPADuplicateDataFilterController", @"sp_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSPADuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPADuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
