...
@interface MKSPADeviceDataController ()<UITableViewDelegate,
...
@implementation MKSPADeviceDataController

- (void)dealloc {
    NSLog(@"MKSPADeviceDataController销毁");
...
- (void)rightButtonMethod {
    MKSPASettingController *vc = [[MKSPASettingController alloc] init];
...
- (void)sp_updateLoadButtonAction {
    MKSPAUploadOptionController *vc = [[MKSPAUploadOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)sp_scannerStatusChanged:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSPAMQTTInterface spa_configScanSwitchStatus:isOn
...
- (void)sp_saveButtonAction {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSPAMQTTInterface spa_configScanSwitchStatus:self.headerModel.isOn
                                          scanTime:[self.headerModel.scanTime integerValue]
                                          deviceID:[MKSPDeviceModeManager shared].deviceID
                                        macAddress:[MKSPDeviceModeManager shared].macAddress
                                             topic:[MKSPDeviceModeManager shared].subscribedTopic
...
- (void)readDataFromServer {
    ...
    [MKSPAMQTTInterface spa_readScanSwitchPramsWithDeviceID:[MKSPDeviceModeManager shared].deviceID
                                                 macAddress:[MKSPDeviceModeManager shared].macAddress
                                                      topic:[MKSPDeviceModeManager shared].subscribedTopic
                                                   sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
    ...
    if (self.headerModel.isOn) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveDeviceDatas:)
                                                     name:MKSPAReceiveDeviceDatasNotification
                                                   object:nil];
        return;
    }
    //关闭状态
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPADeviceDataController", @"sp_moreIcon.png") forState:UIControlStateNormal];
