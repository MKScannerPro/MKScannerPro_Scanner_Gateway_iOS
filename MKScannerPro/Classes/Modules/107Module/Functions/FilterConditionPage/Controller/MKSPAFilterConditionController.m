//
//  MKSPAFilterConditionController.m
...
@interface MKSPAFilterConditionController ()
<UITableViewDelegate,
...
@property (nonatomic, strong)MKSPAFilterConditionModel *dataModel;

@end

@implementation MKSPAFilterConditionController

- (void)dealloc {
    NSLog(@"MKSPAFilterConditionController销毁");
}
...
- (void)rightButtonMethod {
    ...
    if (self.dataModel.rawDataIson) {
        for (NSInteger i = 0; i < self.section3List.count; i ++) {
            ...
            MKSPAFilterRawAdvDataModel *model = [[MKSPAFilterRawAdvDataModel alloc] init];
...
- (void)loadSection4Datas {
    ...
    cellModel.index = 0;
    NSString *conditionType = (self.conditionType == mk_spa_conditionType_A) ? @"A" : @"B";
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = (self.conditionType == mk_spa_conditionType_A) ? @"Filter Condition A" : @"Filter Condition B";
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPAFilterConditionController", @"sp_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSPAFilterConditionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPAFilterConditionModel alloc] init];
    }
    return _dataModel;
}
