...
/**
 线程ID
 */
@property (nonatomic, assign)mk_spa_serverOperationID operationID;
...
@implementation MKSPAMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKSPAMQTTOperation销毁");
}
...
#pragma mark - MKSPAMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKSPAMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_spa_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_spa_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
