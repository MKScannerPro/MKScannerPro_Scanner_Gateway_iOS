...
@implementation MKSPFilterBeaconCellModel
@end

@interface MKSPFilterBeaconCell ()
...
@implementation MKSPFilterBeaconCell

+ (MKSPFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKSPFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSPFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKSPFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSPFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKSPFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_sp_beaconMinValueChanged:index:)]) {
                [self.delegate mk_sp_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_sp_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_sp_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
