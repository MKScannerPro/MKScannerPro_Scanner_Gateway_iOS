...
@interface MKSPFilterByRawDataCellModel : NSObject
...
typedef NS_ENUM(NSInteger, mk_sp_filterRawAdvDataTextType) {
    mk_sp_filterRawAdvDataTextTypeDataType,            //过滤类型输入框内容发生改变
    mk_sp_filterRawAdvDataTextTypeMinIndex,            //开始过滤的Byte索引输入框发生改变
    mk_sp_filterRawAdvDataTextTypeMaxIndex,            //截止过滤的Byte索引输入框发生改变
    mk_sp_filterRawAdvDataTextTypeRawDataType,         //过滤内容输入框发生改变
};

@protocol MKSPFilterByRawDataCellDelegate <NSObject>
...
@interface MKSPFilterByRawDataCell : MKBaseCell

@property (nonatomic, strong)MKSPFilterByRawDataCellModel *dataModel;

@property (nonatomic, weak)id <MKSPFilterByRawDataCellDelegate>delegate;
