...
@interface MKSPPDeviceDataController ()<UITableViewDelegate,
...
@implementation MKSPPDeviceDataController

- (void)dealloc {
    NSLog(@"MKSPPDeviceDataController销毁");
...
- (void)serverManagerStateChanged {
    if ([MKSPPMQTTManager shared].state != MKSPMQTTSessionManagerStateConnected) {
        [[NSNotificationCenter defaultCenter] removeObserver:self];
    ...
    if (self.headerModel.isOn) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveDeviceDatas:)
                                                     name:MKSPPReceiveDeviceDatasNotification
                                                   object:nil];
        return;
    }
    //关闭状态
