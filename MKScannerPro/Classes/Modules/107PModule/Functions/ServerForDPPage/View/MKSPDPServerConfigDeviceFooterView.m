//
//  MKSPDPServerConfigDeviceFooterView.m
...
@implementation MKSPDPServerConfigDeviceFooterViewModel
@end
...
@interface MKSPDPServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKSPDPMQTTSSLForDeviceViewDelegate,
MKSPDPServerConfigDeviceSettingViewDelegate>
...
@property (nonatomic, strong)MKSPDPMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKSPDPServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKSPDPServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_qosChanged:)]) {
        [self.delegate spdp_mqtt_serverForDevice_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}
...
- (void)spdp_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)spdp_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate spdp_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)spdp_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spdp_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)spdp_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spdp_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)spdp_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spdp_mqtt_serverForDevice_fileButtonPressed:2];
    }
}
...
- (void)spdp_mqtt_deviecSetting_deviceIDChanged:(NSString *)deviceID {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_textFieldValueChanged:deviceID textID:3];
    }
}
...
- (void)spdp_mqtt_deviecSetting_ntpURLChanged:(NSString *)url {
    if ([self.delegate respondsToSelector:@selector(sp_mqtta_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spdp_mqtt_serverForDevice_textFieldValueChanged:url textID:4];
    }
}

- (void)spdp_mqtt_deviecSetting_timeZoneChanged:(NSInteger)timeZone {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_timeZoneChanged:)]) {
        [self.delegate spdp_mqtt_serverForDevice_timeZoneChanged:timeZone];
    }
}

- (void)spdp_mqtt_deviecSetting_domainChanged:(NSInteger)domain {
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_serverForDevice_domainChanged:)]) {
        [self.delegate spdp_mqtt_serverForDevice_domainChanged:domain];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKSPDPServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPDPServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKSPDPMQTTSSLForDeviceViewModel *sslModel = [[MKSPDPMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKSPDPServerConfigDeviceSettingViewModel *settingModel = [[MKSPDPServerConfigDeviceSettingViewModel alloc] init];
...
- (MKSPDPMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSPDPMQTTSSLForDeviceView alloc] init];
...
- (MKSPDPServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKSPDPServerConfigDeviceSettingView alloc] init];
