//
//  MKSPDPServerConfigDeviceSettingView.m
...
@implementation MKSPDPServerConfigDeviceSettingViewModel
@end

@interface MKSPDPServerConfigDeviceSettingView ()
...
@implementation MKSPDPServerConfigDeviceSettingView
...
- (void)timeZoneButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.timeZoneList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(spdp_mqtt_deviecSetting_timeZoneChanged:)]) {
            [self.delegate spdp_mqtt_deviecSetting_timeZoneChanged:currentRow];
        }
    }];
}
...
- (void)domainButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.domainList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(spdp_mqtt_deviecSetting_domainChanged:)]) {
            [self.delegate spdp_mqtt_deviecSetting_domainChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPDPServerConfigDeviceSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPDPServerConfigDeviceSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)deviceIDField {
    if (!_deviceIDField) {
        ...
        _deviceIDField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spdp_mqtt_deviecSetting_deviceIDChanged:)]) {
                [self.delegate spdp_mqtt_deviecSetting_deviceIDChanged:text];
            }
        };
        ...
        _deviceIDField.textColor = DEFAULT_TEXT_COLOR;
...
- (MKTextField *)ntpUrlField {
    if (!_ntpUrlField) {
        ...
        _ntpUrlField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spdp_mqtt_deviecSetting_ntpURLChanged:)]) {
                [self.delegate spdp_mqtt_deviecSetting_ntpURLChanged:text];
            }
        };
        ...
        _ntpUrlField.textColor = DEFAULT_TEXT_COLOR;
