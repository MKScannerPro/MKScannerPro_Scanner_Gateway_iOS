...
@implementation MKSPDPMQTTSSLForDeviceViewModel
@end

@interface MKSPDPMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKSPDPMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(spdp_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate spdp_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(spdp_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate spdp_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate spdp_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(spdp_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate spdp_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(spdp_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate spdp_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPDPMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPDPMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKScannerPro", @"MKSPDPMQTTSSLForDeviceView", @"sp_switchSelectedIcon.png") : LOADICON(@"MKScannerPro", @"MKSPDPMQTTSSLForDeviceView", @"sp_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKScannerPro", @"MKSPDPMQTTSSLForDeviceView", @"sp_switchUnselectedIcon.png") forState:UIControlStateNormal];
