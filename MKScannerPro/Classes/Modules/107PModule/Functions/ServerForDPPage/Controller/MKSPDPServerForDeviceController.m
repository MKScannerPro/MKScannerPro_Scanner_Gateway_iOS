//
//  MKSPDPServerForDeviceController.m
...
@interface MKSPDPServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKSPDPServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKSPDPServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKSPDPServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKSPDPServerConfigDeviceFooterViewModel *sslParamsModel;
...
@implementation MKSPDPServerForDeviceController

- (void)dealloc {
    NSLog(@"MKSPDPServerForDeviceController销毁");
...
#pragma mark - MKSPDPServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)spdp_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)spdp_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)spdp_mqtt_serverForDevice_qosChanged:(NSInteger)qos {
...
- (void)spdp_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)spdp_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)spdp_mqtt_serverForDevice_timeZoneChanged:(NSInteger)timeZone {
...
/// 频段改变
/// @param timeZone 频段
- (void)spdp_mqtt_serverForDevice_domainChanged:(NSInteger)domain {
    self.dataModel.domain = domain;
    self.sslParamsModel.domain = domain;
}
...
- (void)configServerParamsComplete {
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                  ...
                                                  object:nil];
    NSString *topic = @"";
    if (ValidStr([MKSPPMQTTManager shared].subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKSPPMQTTManager shared].subscribeTopic;
    }else {
        topic = self.dataModel.publishTopic;
    }
    [self.progressView show];
    [[MKSPPMQTTManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)loadFooterViewDatas {
    ...
    self.sslParamsModel.domain = self.dataModel.domain;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPPServerForDeviceController", @"sp_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSPDPServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPDPServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSPDPServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSPDPServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKSPDPServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSPDPServerConfigDeviceFooterViewModel alloc] init];
    }
