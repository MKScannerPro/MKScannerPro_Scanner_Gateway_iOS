...
@interface MKSPDPServerForDeviceModel ()
...
@implementation MKSPDPServerForDeviceModel

- (instancetype)init {
    if (self = [super init]) {
        _host = [MKSPPMQTTManager shared].currentServerParams.host;
        _port = [MKSPPMQTTManager shared].currentServerParams.port;
...
- (void)readPramsFromDeviceWithSucBlock:(void (^)(void))sucBlock
    ...
    dispatch_async(self.configQueue, ^{
        ...
        if (![self readChannel]) {
            [self operationFailedBlockWithMsg:@"Read Channel Error" block:failedBlock];
            return;
        }
...
- (void)configParamsWithWifiSSID:(NSString *)ssid
    ...
    dispatch_async(self.configQueue, ^{
        ...
        if (![self configChannel]) {
            [self operationFailedBlockWithMsg:@"Config Channel Error" block:failedBlock];
            return;
        }
...
- (BOOL)configTimeZone {
    __block BOOL success = NO;
    [MKSPInterface sp_configPTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configChannel {
    __block BOOL success = NO;
    [MKSPInterface sp_configChannel:self.domain sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readChannel {
    __block BOOL success = NO;
    [MKSPInterface sp_readChannelWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
        self.domain = [returnData[@"result"][@"domain"] integerValue];
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
    }];
