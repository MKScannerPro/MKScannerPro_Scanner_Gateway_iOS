...
- (BOOL)readNTPServer {
    ...
    [MKSPPMQTTInterface spp_readNTPServerWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNTPServer {
    __block BOOL success = NO;
    [MKSPPMQTTInterface spp_configNTPServer:self.isOn host:self.host deviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
