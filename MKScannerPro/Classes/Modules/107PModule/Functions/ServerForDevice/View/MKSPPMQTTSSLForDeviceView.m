...
@implementation MKSPPMQTTSSLForDeviceViewModel
@end

@interface MKSPPMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKSPPMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(spp_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate spp_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(spp_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate spp_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate spp_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate spp_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(spp_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate spp_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPPMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPPMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKScannerPro", @"MKSPPMQTTSSLForDeviceView", @"sp_switchSelectedIcon.png") : LOADICON(@"MKScannerPro", @"MKSPPMQTTSSLForDeviceView", @"sp_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKScannerPro", @"MKSPPMQTTSSLForDeviceView", @"sp_switchUnselectedIcon.png") forState:UIControlStateNormal];
