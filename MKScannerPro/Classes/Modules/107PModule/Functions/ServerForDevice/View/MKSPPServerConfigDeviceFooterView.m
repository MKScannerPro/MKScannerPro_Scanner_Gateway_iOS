//
//  MKSPPServerConfigDeviceFooterView.m
...
@implementation MKSPPServerConfigDeviceFooterViewModel
@end
...
@interface MKSPPServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKSPPMQTTSSLForDeviceViewDelegate,
MKSPPServerConfigDeviceSettingViewDelegate>
...
@property (nonatomic, strong)MKSPPMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKSPPServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKSPPServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate spp_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_qosChanged:)]) {
        [self.delegate spp_mqtt_serverForDevice_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKSPPMQTTSSLForDeviceViewDelegate
- (void)spp_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate spp_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA signed server certificate     1:CA certificate     2:Self signed certificates
- (void)spp_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate spp_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)spp_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spp_mqtt_serverForDevice_fileButtonPressed:0];
    }
}

/// 用户点击选择了Client Key按钮
- (void)spp_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spp_mqtt_serverForDevice_fileButtonPressed:1];
    }
}

/// 用户点击了Client Cert File按钮
- (void)spp_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate spp_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKSPPServerConfigDeviceSettingViewDelegate
- (void)spp_mqtt_deviecSetting_deviceIDChanged:(NSString *)deviceID {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_serverForDevice_textFieldValueChanged:deviceID textID:3];
    }
}

- (void)spp_mqtt_deviecSetting_ntpURLChanged:(NSString *)url {
    if ([self.delegate respondsToSelector:@selector(sp_mqtta_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_serverForDevice_textFieldValueChanged:url textID:4];
    }
}

- (void)spp_mqtt_deviecSetting_timeZoneChanged:(NSInteger)timeZone {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_serverForDevice_timeZoneChanged:)]) {
        [self.delegate spp_mqtt_serverForDevice_timeZoneChanged:timeZone];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKSPPServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPPServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKSPPMQTTSSLForDeviceViewModel *sslModel = [[MKSPPMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKSPPServerConfigDeviceSettingViewModel *settingModel = [[MKSPPServerConfigDeviceSettingViewModel alloc] init];
...
- (MKSPPMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSPPMQTTSSLForDeviceView alloc] init];
...
- (MKSPPServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKSPPServerConfigDeviceSettingView alloc] init];
