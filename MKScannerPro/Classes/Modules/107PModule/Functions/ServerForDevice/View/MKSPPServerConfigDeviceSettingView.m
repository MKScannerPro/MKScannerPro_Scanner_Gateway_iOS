//
//  MKSPPServerConfigDeviceSettingView.m
...
@implementation MKSPPServerConfigDeviceSettingViewModel
@end

@interface MKSPPServerConfigDeviceSettingView ()
...
@implementation MKSPPServerConfigDeviceSettingView
...
- (void)timeZoneButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(spp_mqtt_deviecSetting_timeZoneChanged:)]) {
            [self.delegate spp_mqtt_deviecSetting_timeZoneChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSPPServerConfigDeviceSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPPServerConfigDeviceSettingViewModel.class]) {
        return;
    }
    ...
    [self.timeZoneButton setTitle:self.dataList[_dataModel.timeZone] forState:UIControlStateNormal];
}
...
- (MKTextField *)deviceIDField {
    if (!_deviceIDField) {
        ...
        _deviceIDField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spp_mqtt_deviecSetting_deviceIDChanged:)]) {
                [self.delegate spp_mqtt_deviecSetting_deviceIDChanged:text];
            }
        };
...
- (MKTextField *)ntpUrlField {
    if (!_ntpUrlField) {
        ...
        _ntpUrlField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spp_mqtt_deviecSetting_ntpURLChanged:)]) {
                [self.delegate spp_mqtt_deviecSetting_ntpURLChanged:text];
            }
        };
