//
//  MKSPPServerForDeviceController.m
...
@interface MKSPPServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKSPPServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKSPPServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKSPPServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKSPPServerConfigDeviceFooterViewModel *sslParamsModel;
...
@implementation MKSPPServerForDeviceController

- (void)dealloc {
    NSLog(@"MKSPPServerForDeviceController销毁");
...
#pragma mark - MKSPPServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)spp_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)spp_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)spp_mqtt_serverForDevice_qosChanged:(NSInteger)qos {
...
- (void)spp_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)spp_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)spp_mqtt_serverForDevice_timeZoneChanged:(NSInteger)timeZone {
...
- (MKSPPServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPPServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSPPServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSPPServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKSPPServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSPPServerConfigDeviceFooterViewModel alloc] init];
    }
