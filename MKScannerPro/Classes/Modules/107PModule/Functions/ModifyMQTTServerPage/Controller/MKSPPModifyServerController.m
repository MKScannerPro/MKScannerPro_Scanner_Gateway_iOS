...
MKSPPModifyServerFooterViewDelegate>
...
@property (nonatomic, strong)MKSPPModifyServerModel *dataModel;

@property (nonatomic, strong)MKSPPModifyServerFooterView *sslParamsView;

@property (nonatomic, strong)MKSPPModifyServerFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKSPPModifyServerController

- (void)dealloc {
    NSLog(@"MKSPPModifyServerController销毁");
...
#pragma mark - MKSPPModifyServerFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
#pragma mark - 入网
/*
 1、先发送更新的服务器信息给设备，设备接收到之后会处理相关逻辑，当设备上报3005的时候表明已经处理完毕。
 2、如果上报3005的通知里面的下载结果成功，则需要发送设备重入网指令给设备
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPPModifyServerController", @"sp_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSPPModifyServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPPModifyServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSPPModifyServerFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSPPModifyServerFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKSPPModifyServerFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSPPModifyServerFooterViewModel alloc] init];
    }
