//
//  MKSPPModifyServerSSLTextField.m
...
@implementation MKSPPModifyServerSSLTextFieldModel
@end

@interface MKSPPModifyServerSSLTextField ()
...
@implementation MKSPPModifyServerSSLTextField
...
#pragma mark - setter
- (void)setDataModel:(MKSPPModifyServerSSLTextFieldModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPPModifyServerSSLTextFieldModel.class]) {
        return;
    }
    ...
    if (!_textField) {
        ...
        _textField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            if ([self.delegate respondsToSelector:@selector(spp_modifyServerSSLTextFieldValueChanged:textValue:)]) {
                [self.delegate spp_modifyServerSSLTextFieldValueChanged:self.dataModel.index textValue:text];
            }
        };
    }
    return _textField;
