//
//  MKSPPModifyServerSettingView.m
...
@implementation MKSPPModifyServerSettingViewModel
@end

@interface MKSPPModifyServerSettingView ()
...
@implementation MKSPPModifyServerSettingView
...
#pragma mark - setter
- (void)setDataModel:(MKSPPModifyServerSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPPModifyServerSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)ssidField {
    if (!_ssidField) {
        ...
        _ssidField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyDevice_wifiSSIDChanged:)]) {
                [self.delegate spp_mqtt_modifyDevice_wifiSSIDChanged:text];
            }
        };
...
- (MKTextField *)passwordField {
    if (!_passwordField) {
        ...
        _passwordField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyDevice_wifiPasswordChanged:)]) {
                [self.delegate spp_mqtt_modifyDevice_wifiPasswordChanged:text];
            }
        };
