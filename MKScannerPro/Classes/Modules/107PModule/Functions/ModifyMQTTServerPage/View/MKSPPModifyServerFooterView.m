...
@implementation MKSPPModifyServerFooterViewModel
@end

static CGFloat const buttonWidth = 100.f;
static CGFloat const buttonHeight = 30.f;
static CGFloat const settingViewHeight = 100.f;
static CGFloat const defaultScrollViewHeight = 270.f;

@interface MKSPPModifyServerFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKSPPModifyServerSSLViewDelegate,
MKSPPModifyServerSettingViewDelegate>
...
@property (nonatomic, strong)MKSPPModifyServerSSLView *sslView;

@property (nonatomic, strong)MKSPPModifyServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKSPPModifyServerFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_qosChanged:)]) {
        [self.delegate spp_mqtt_modifyMQTT_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKSPPModifyServerSSLViewDelegate
- (void)spp_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA signed server certificate     1:CA certificate     2:Self signed certificates
- (void)spp_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_certificateChanged:)]) {
        [self.delegate spp_mqtt_modifyMQTT_certificateChanged:certificate];
    }
}
...
/// @param value value
- (void)spp_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_textFieldValueChanged:value textID:(index + 5)];
    }
}

#pragma mark - MKSPPModifyServerSettingViewDelegate
- (void)spp_mqtt_modifyDevice_wifiSSIDChanged:(NSString *)ssid {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_textFieldValueChanged:ssid textID:3];
    }
}

- (void)spp_mqtt_modifyDevice_wifiPasswordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(spp_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate spp_mqtt_modifyMQTT_textFieldValueChanged:password textID:4];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKSPPModifyServerFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPPModifyServerFooterViewModel.class]) {
        return;
    }
    ...
    MKSPPModifyServerSSLViewModel *sslModel = [[MKSPPModifyServerSSLViewModel alloc] init];
    ...
    MKSPPModifyServerSettingViewModel *settingModel = [[MKSPPModifyServerSettingViewModel alloc] init];
    settingModel.password = _dataModel.wifiPassword;
    settingModel.ssid = _dataModel.wifiSSID;
...
- (MKSPPModifyServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSPPModifyServerSSLView alloc] init];
...
- (MKSPPModifyServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKSPPModifyServerSettingView alloc] init];
