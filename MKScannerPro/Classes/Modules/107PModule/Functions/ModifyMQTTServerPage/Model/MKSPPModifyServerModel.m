...
@implementation MKSPPUpdateMQTTServerModel

- (instancetype)initWithModifyServerModel:(MKSPPModifyServerModel *)serverModel {
    if (self = [self init]) {
...
@interface MKSPPModifyServerModel ()
...
@implementation MKSPPModifyServerModel

- (void)dealloc {
    NSLog(@"MKSPPModifyServerModel销毁");
...
- (void)updateServerWithDeviceID:(NSString *)deviceID
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDownloadDatasResult:)
                                                 name:MKSPPSSLCertificateDownloadResultsNotification
                                               object:nil];
...
- (void)receiveDownloadDatasResult:(NSNotification *)note {
    ...
    [MKSPPMQTTInterface spp_reconnectNetworkWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        if (self.sucBlock) {
            self.sucBlock();
        }
