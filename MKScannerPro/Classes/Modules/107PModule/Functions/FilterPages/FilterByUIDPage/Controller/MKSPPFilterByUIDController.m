...
@interface MKSPPFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSPPFilterByUIDModel *dataModel;

@end

@implementation MKSPPFilterByUIDController

- (void)dealloc {
    NSLog(@"MKSPPFilterByUIDController销毁");
}
...
- (void)mk_sp_filterNormalTextFieldValueChanged:(NSString *)text index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKSPFilterNormalTextFieldCellModel *cellModel = self.section1List[1];
        cellModel.textFieldValue = text;
        return;
    }
}
...
- (void)loadSection1Datas {
    MKSPFilterNormalTextFieldCellModel *cellModel1 = [[MKSPFilterNormalTextFieldCellModel alloc] init];
    cellModel1.index = 0;
    ...
    MKSPFilterNormalTextFieldCellModel *cellModel2 = [[MKSPFilterNormalTextFieldCellModel alloc] init];
    cellModel2.index = 1;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKScannerPro", @"MKSPPFilterByUIDController", @"sp_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSPPFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSPPFilterByUIDModel alloc] initWithDeviceID:self.deviceModel.deviceID
