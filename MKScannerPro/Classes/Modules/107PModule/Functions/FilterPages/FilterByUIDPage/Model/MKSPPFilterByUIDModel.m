...
- (BOOL)readFilterByUID {
    ...
    [MKSPPMQTTInterface spp_readFilterByUIDWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByUID {
    __block BOOL success = NO;
    [MKSPPMQTTInterface spp_configFilterByUID:self.isOn namespaceID:self.namespaceID instanceID:self.instanceID deviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
