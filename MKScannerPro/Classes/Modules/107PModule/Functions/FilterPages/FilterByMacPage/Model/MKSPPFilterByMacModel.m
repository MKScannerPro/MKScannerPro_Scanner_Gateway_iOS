...
- (BOOL)readFilterByMac {
    ...
    [MKSPPMQTTInterface spp_readFilterByMacAddressWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByMac:(NSArray <NSString *>*)macList {
    __block BOOL success = NO;
    [MKSPPMQTTInterface spp_configFilterByMacAddress:macList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter deviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
