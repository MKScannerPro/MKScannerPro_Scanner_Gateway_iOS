...
static NSString *const mk_sp_logName = @"mk_sp_bleLog";

NSString *const mk_sp_peripheralConnectStateChangedNotification = @"mk_sp_peripheralConnectStateChangedNotification";
NSString *const mk_sp_centralManagerStateChangedNotification = @"mk_sp_centralManagerStateChangedNotification";

NSString *const mk_sp_deviceDisconnectTypeNotification = @"mk_sp_deviceDisconnectTypeNotification";
...
@property (nonatomic, assign)mk_sp_centralConnectStatus connectStatus;

@end

@implementation MKSPCentralManager

- (void)dealloc {
    [self logToLocal:@"MKSPCentralManager销毁"];
    NSLog(@"MKSPCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKSPCentralManager初始化"];
...
+ (MKSPCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKSPCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_sp_receiveDevice:)]) {
                [self.delegate mk_sp_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_sp_startScan)]) {
        [self.delegate mk_sp_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_sp_stopScan)]) {
        [self.delegate mk_sp_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_sp_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_sp_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_sp_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_sp_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_sp_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_sp_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_sp_deviceDisconnectTypeNotification
                                                            object:nil
...
- (mk_sp_centralManagerStatus )centralStatus {
    ...
    ? mk_sp_centralManagerStatusEnable
    : mk_sp_centralManagerStatusUnable;
}

- (void)startScan {
    [[MKBLEBaseCentralManager shared] scanForPeripheralsWithServices:@[[CBUUID UUIDWithString:@"AA03"]] options:nil];
}
...
- (void)addTaskWithTaskID:(mk_sp_taskOperationID)operationID
    ...
    MKSPOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_sp_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKSPOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKSPPeripheral *trackerPeripheral = [[MKSPPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    MKSPOperation *operation = [[MKSPOperation alloc] initOperationWithID:mk_sp_connectPasswordOperation commandBlock:^{
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_sp_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_sp_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKSPOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_sp_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKSPOperation <MKBLEBaseOperationProtocol>*operation = [[MKSPOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKSPOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_sp_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (NSDictionary *)parseModelWithRssi:(NSNumber *)rssi advDic:(NSDictionary *)advDic peripheral:(CBPeripheral *)peripheral {
    ...
    NSData *manufacturerData = manuParams[[CBUUID UUIDWithString:@"AA03"]];
    NSData *normalData = advDic[CBAdvertisementDataManufacturerDataKey];
    if (!MKValidData(manufacturerData) || manufacturerData.length != 1
        || !MKValidData(normalData) || normalData.length < 14) {
        return @{};
    }
    NSString *content = [MKBLEBaseSDKAdopter hexStringFromData:[normalData subdataWithRange:NSMakeRange(8, 6)]];
    
    NSString *deviceType = [MKBLEBaseSDKAdopter hexStringFromData:manufacturerData];
    NSString *firmware = [[NSString alloc] initWithData:[normalData subdataWithRange:NSMakeRange(2, 6)] encoding:NSUTF8StringEncoding];
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_sp_logName dataList:@[string]];
}
