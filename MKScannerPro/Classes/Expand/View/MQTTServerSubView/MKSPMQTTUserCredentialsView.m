...
@implementation MKSPMQTTUserCredentialsViewModel
@end

@interface MKSPMQTTUserCredentialsView ()
...
@implementation MKSPMQTTUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKSPMQTTUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSPMQTTUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(sp_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate sp_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(sp_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate sp_mqtt_userCredentials_passwordChanged:text];
            }
        };
